...
@onready var classic_scroll = $MainMargin/LayoutList/SectionClassic/ContainerClassic
...
func setup_classic_mode():
	var total_levels = Global.game_levels.size()
	var desired_columns = int(ceil(float(total_levels) / 2.0))
	grid_classic.columns = max(3, desired_columns)
	classic_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_AUTO
	classic_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
