...
# Touch shader pulse effect
var touch_shader_pos = Vector2.ZERO
var touch_shader_alpha = 0.0
var touch_shader_radius = 0.0
...
func _process(delta):
	...
	if touch_shader_alpha > 0:
		touch_shader_alpha = max(0.0, touch_shader_alpha - (2.2 * delta))
		touch_shader_radius += 260.0 * delta
...
func _input(event):
	...
	if show_results_screen:
		if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			touch_shader_pos = event.position
			touch_shader_alpha = 0.9
			touch_shader_radius = 16.0
...
func draw_touch_shader_effect():
	if touch_shader_alpha <= 0.0:
		return
	var base_col = COLOR_ACTIVE_PLAYER
	base_col.a = 0.18 * touch_shader_alpha
	draw_circle(touch_shader_pos, touch_shader_radius, base_col)
	var ring_col = Color.WHITE
	ring_col.a = 0.45 * touch_shader_alpha
	draw_arc(touch_shader_pos, touch_shader_radius + 8.0, 0.0, TAU, 42, ring_col, 2.0)
	var ring_col_2 = COLOR_GOLD
	ring_col_2.a = 0.22 * touch_shader_alpha
	draw_arc(touch_shader_pos, touch_shader_radius + 20.0, 0.0, TAU, 42, ring_col_2, 1.0)
